...
void
qr_mode (char *mode, int ver, int len, const char *input)
...
{                               // Return (malloced) byte array width*width wide (includes mandatory quiet zone)
...
#ifdef DEBUG
      fprintf (stderr, "ECC blocks=%d total=%d size=%d data=%d %d*%d %d*%d\n", blocks, ecctotal, eccsize, total,
               datas, datan, datas + 1, blocks - datan);
#endif
      int p = 0,
         q = 0;
      for (q = 0; q < datas; q++)
   ...
   memset (grid, 0, (w + 8) * (w + 8));
   inline void set (int x, int y, int v)
   {
      if (x >= 0 && x < w && y >= 0 && y < w)
         grid[(w + 8) * (y + 4) + (x + 4)] = (v | 128);
   }
      ...
      {                         // Auto mask
         ...
         for (m = 0; m < 8; m++)
         {
            ...
            for (x = 0; x < w; x++)
               ...
               {
                  int bit (int x, int y)
                  {             // What colour would a bit be, masked if data
                     int v = grid[(w + 8) * (y + 4) + (x + 4)];
                     if (v & 4)
                        v ^= getmask (x, y, m);
                     return v & 1;
                  }
